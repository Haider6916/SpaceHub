...
import {ArrowDown, CheckedIn, ThreeDots} from '../../assets';
import {
  DeactivateModal,
  DeleteModal,
  Status,
...
function CompanyCard(props, navigation) {
  const {
    image,
    imgStyle,
    Title,
  ...
  const options = [
    {id: 1, label: 'View Company'},
    {id: 2, label: 'Update Company'},
  ];
    ...
    <Pressable style={styles.container} onPress={() => setShowDropdown(false)}>
      <View style={{flexDirection: 'row'}}>
        ...
        <View style={{marginTop: 24, flex: 0.95, paddingLeft: 24}}>
          ...
          <Text
            regular
            body1
            style={{lineHeight: 24, color: 'rgba(159, 157, 158, 1)'}}
            numberOfLines={1}>
            {description}
          </Text>
        </View>
      ...
      <View style={{flexDirection: 'row', alignItems: 'center'}}>
        <View style={{flex: 0.98, marginLeft: 10}}>
          ...
          <Text body1 regular style={{lineHeight: 24}} secondaryColor>
            {BottomSubText}
          </Text>
        </View>
        <View style={styles.statusContainer}>
          {Check ? (
            <View style={{width:110,height:32,borderRadius:50,justifyContent:'center',alignItems:'center',backgroundColor:'rgb(254, 242, 223)'}}>
